...
.define(function(options,wda){
  ...
  plugin.startIproxy = function(localPort,remotePort){
    ...
    let pro = new Subprocess("iproxy",["-s","0.0.0.0","-u",options.serial,localPort,remotePort])
    pro.start();
  ...
  plugin.restartWda = function(){
    if (!exit && bRestart){
      ...
      /*if(bRestartCnt>3){
        ...
        var uninstall = new Subprocess("ideviceinstaller",["--udid",options.serial,
          "--uninstall","com.apple.test.WebDriverAgentRunner-Runner"])
        uninstall.start()
        var reboot = new Subprocess("idevicediagnostics",["restart","-u",options.serial])
        reboot.start()
        bRestart = false
        plugin.end()
      }
